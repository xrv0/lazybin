...
function publishPaste(req, res) {
    ...
    let paste = {
        ...
        id: id,
        highlighting: req.body.enable_highlighting === "on"
    };
    ...
    fs.readFile(pasteDir + id, (error, data) => {
        if(!error) {
            ...
            if(isNaN(expirationDate.getTime()) || expirationDate.getTime() > new Date().getTime()) {
                if(pasteHighlighting === "true") {
                    res.end(pasteTemplate.replace("$id", id).replace("$paste_content", "<pre><code>" + pasteContent.toString() + "</code></pre>"))
                }else {
                    res.end(pasteTemplate.replace("$id", id).replace("$paste_content", pasteContent.toString()))
                }
                return;
            }else {
